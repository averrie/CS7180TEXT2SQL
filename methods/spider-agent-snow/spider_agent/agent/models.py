...
def call_llm(payload):
    ...
    #     return False, code_value
        ...
        # Extract the actual model name after "openrouter/"
        openrouter_model = model.split("openrouter/")[1]
        
        headers = {
            'Authorization': f'Bearer {os.environ["OPENROUTER_API_KEY"]}',
            'HTTP-Referer': 'https://spider2-benchmark.com',  # Optional but recommended
            'X-Title': 'Spider2-Agent',  # Optional 
            'Content-Type': 'application/json'
        }
        ...
        for i in range(3):
            try:
                ...
                logger.info(f"OpenRouter response code: {response.status_code}")
                ...
                if response.status_code == 200:
                    ...
                    logger.error("Retrying in a few seconds...")
                    time.sleep(10 * (2 ** (i + 1)))
            except Exception as e:
                logger.error(f"Failed to call OpenRouter: {str(e)}")
                time.sleep(10 * (2 ** (i + 1)))
        
        return False, "Failed to get response from OpenRouter after multiple attempts"
