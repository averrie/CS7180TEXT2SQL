...
class Spider_Agent_Env(gym.Env):
    ...
    def __init__(self, env_config, task_config, cache_dir, mnt_dir):
        ...
        self.controller = PythonController(
            container=self.container, work_dir=self.work_dir
        )
        self.setup_controller = SetupController(
            container=self.container, cache_dir=self.cache_dir
        )
    ...
    def _construct_container(self):
        ...
        try:
            ...
            if os.path.exists(dockerfile_path):
                ...
                logger.info(
                    f"Image {self.image_name} not found, try to pull from Dockerhub ..."
                )
            ...
            logger.info(
                f"Failed to construct container from image {self.image_name} with error: {e}"
            )
            raise e
    ...
    def post_process(self):
        ...
        for post_process_f in self.post_process_func:
            ...
            post_files = (
                post_files if isinstance(post_files, list) else list(post_files)
            )
